...
class PregameRoomViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, MultipeerManagerDelegate, PregameRoomViewCellDelegate {
    ...
    @IBAction func onStartGame(sender: AnyObject) {
        if let canStartGame = self.room?.canStartGame() where canStartGame {
            // Go to cluegiver decision and game room
        } else {
            let alertController = UIAlertController(title: "Cannot Start Game", message: CodenamesMessage.cannotStartGameString, preferredStyle: .Alert)
    ...
    override func viewDidLoad() {
        ...
        if let isHost = self.player?.isHost() where isHost, let name = self.room?.getRoomName() {
            ...
            if let peerID = self.multipeerManager?.getPeerID(), playerUUID = self.player?.getPlayerUUID() {
                // Host should add itself to the connected peers
                self.connectedPeers[peerID] = playerUUID
            }
        ...
        if let name = self.room?.getRoomName() {
    ...
    private func broadcastData() {
        if let room = self.room {
            let data = NSKeyedArchiver.archivedDataWithRootObject(room)
            self.multipeerManager?.broadcastData(data)
        }
        
        let data = NSKeyedArchiver.archivedDataWithRootObject(self.connectedPeers)
        self.multipeerManager?.broadcastData(data)
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let playerAtIndex = self.room?.getPlayers()[indexPath.row] {
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if let number = self.room?.getNumberOfPlayers() {
            return number
        } else {
            return 0
        }
    ...
    func foundPeer(peerID: MCPeerID, withDiscoveryInfo info: [String : String]?) {
        if let info = info where info["joinRoom"] == self.room?.getRoomName() {
    ...
    func didReceiveData(data: NSData, fromPeer peerID: MCPeerID) {
        if let player = NSKeyedUnarchiver.unarchiveObjectWithData(data) as? Player {
            ...
            if self.room?.getNumberOfPlayers() == 0 {
                self.returnToLobby(reason: CodenamesMessage.hostDisconnectedString)
            }
            else if let uuid = self.player?.getPlayerUUID(), playerInRoom = self.room?.playerWithUUIDInRoom(uuid) where !playerInRoom {
                self.returnToLobby(reason: CodenamesMessage.removedFromRoomString)
            }
        }
    ...
    func teamDidChange(redTeam: Bool) {
        if let uuid = self.player?.getPlayerUUID() {
            if redTeam {
                self.room?.getPlayerWithUUID(uuid)?.setTeam(Team.Red)
            } else {
                self.room?.getPlayerWithUUID(uuid)?.setTeam(Team.Blue)
            }
            self.room?.getPlayerWithUUID(uuid)?.setIsClueGiver(false)
            self.broadcastData()
        }
    }
